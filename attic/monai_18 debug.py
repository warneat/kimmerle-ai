...
class SpineDataset(Dataset):
    ...
    def _load_image(self, study_id, series_id, instance_number, position="current"):
        image_path = os.path.join(self.data_dir, str(
            study_id), str(series_id), f'{instance_number}.dcm')
        logger.info(
            f"Attempting to load {position} image from path: {image_path}")
        if os.path.exists(image_path):
            ...
            logger.info(
                f"{position.capitalize()} image not found for {study_id}, {series_id}, {instance_number}. Applying augmentation.")
    ...
    def __getitem__(self, idx):
        ...
        image = self._load_image(
            study_id, series_id, instance_number, position="current")
        image_above = self._load_image(
            study_id, series_id, instance_number - 1, position="above")
        image_below = self._load_image(
            study_id, series_id, instance_number + 1, position="below")

        images = torch.cat([image_above, image, image_below], dim=0)
...
class SpineStenosisClassifier(nn.Module):
    def __init__(self, num_conditions, num_levels):
        ...
        self.model = DenseNet121(
            spatial_dims=2, in_channels=3, out_channels=num_conditions + num_levels)
