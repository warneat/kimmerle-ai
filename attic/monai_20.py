...
# Load the adapted CSV file
data = pd.read_csv('combined_paths.csv')
data = data[['image_path', 'condition', 'level']]
...
train_dataset = SpineDataset(train_data, transform=transforms)
val_dataset = SpineDataset(val_data, transform=transforms)
...
optimizer = Adam(model.parameters(), lr=5e-4) #0.0005
num_epochs = 80
